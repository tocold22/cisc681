...
function createSierpinskiTetrahedron(level, size, position) {
    if (level === 0) {
        ...
        // Calculate the vertical offset for the new tetrahedrons
        let verticalOffset = height / 2;

        // Calculate the positions for the smaller tetrahedra
        let positions = [
...
function updateSierpinskiTetrahedron() {
    ...
    let size = 10; // Adjust the initial size as needed
    let height = size * Math.sqrt(2 / 3);
    let position = new THREE.Vector3(0, -height / 2, 0); // Adjust position to ensure it's centered
